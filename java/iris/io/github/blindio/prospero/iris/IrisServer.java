...
import io.github.blindio.prospero.iris.exceptions.IrisRuntimeException;
...
public class IrisServer implements SimpleMessageListener {
	...
	Iterator<IrisMessage> iter = getAllMailMailbox()
		.getOldestToNewestIterator();
	while (iter.hasNext()) {
	...
	if (mailboxName != ALLMAIL_MAILBOX_NAME) {
	    ...
	    throw new IrisRuntimeException("Cannot delete mailbox named: "
		    + ALLMAIL_MAILBOX_NAME);
	}
    }
	...
	return serverSingleton != null ? serverSingleton.server.getPort()
		: null;
    }

    public static String getHostName() {
	return serverSingleton != null ? serverSingleton.server.getHostName()
		: null;
    }
}
