...
/**
 * Defines an object that runs Selenium commands. <h3><a name="locators"></a>Element Locators</h3>
 * <p>
 ...
 * </p>
 * <ul>
 * <li><strong>identifier</strong>=<em>id</em>: Select the element with the specified @id attribute. If no
 * match is found, select the first element whose @name attribute is <em>id</em>. (This is normally the
 * default; see below.)</li>
 * <li><strong>id</strong>=<em>id</em>: Select the element with the specified @id attribute.</li>
 * <li><strong>name</strong>=<em>name</em>: Select the first element with the specified @name attribute.
 * <ul class="first last simple">
 * <li>username</li>
 * <li>name=username</li>
 * </ul>
 * <p>
 * The name may optionally be followed by one or more <em>element-filters</em>, separated from the name by
 * whitespace. If the <em>filterType</em> is not specified, <strong>value</strong> is assumed.
 * </p>
 * <ul class="first last simple">
 * <li>name=flavour value=chocolate</li>
 * </ul>
 * </li>
 * <li><strong>dom</strong>=<em>javascriptExpression</em>: Find an element by evaluating the specified string.
 * This allows you to traverse the HTML Document Object Model using JavaScript. Note that you must not return
 * a value in this string; simply make it the last expression in the block.
 * <ul class="first last simple">
 ...
 * </ul>
 * </li>
 * <li><strong>link</strong>=<em>textPattern</em>: Select the link (anchor) element which contains text
 * matching the specified <em>pattern</em>.
 * <ul class="first last simple">
 * <li>link=The link text</li>
 * </ul>
 * </li>
 * <li><strong>css</strong>=<em>cssSelectorSyntax</em>: Select the element using css selectors. Please refer
 * to <a href="http://www.w3.org/TR/REC-CSS2/selector.html">CSS2 selectors</a>, <a
 * href="http://www.w3.org/TR/2001/CR-css3-selectors-20011113/">CSS3 selectors</a> for more information. You
 * can also check the TestCssLocators test in the selenium test suite for an example of usage, which is
 ...
 * </p>
 * </li>
 * <li><strong>ui</strong>=<em>uiSpecifierString</em>: Locate an element by resolving the UI specifier string
 * to another locator, and evaluating it. See the <a href=
 ...
 * <p>
 * Element filters can be used with a locator to refine a list of candidate elements. They are currently used
 * only in the 'name' element-locator.
 * </p>
 * <p>
 ...
 * (offset from zero).</blockquote></blockquote><h3><a name="patterns"></a>String-match Patterns</h3>
 * <p>
 * Various Pattern syntaxes are available for matching string values:
 * </p>
 * <ul>
 * <li><strong>glob:</strong><em>pattern</em>: Match a string against a "glob" (aka "wildmat") pattern. "Glob"
 * is a kind of limited regular-expression syntax typically used in command-line shells. In a glob pattern,
 * "*" represents any sequence of characters, and "?" represents any single character. Glob patterns match
 * against the entire string.</li>
 * <li><strong>regexp:</strong><em>regexp</em>: Match a string using a regular-expression. The full power of
 * JavaScript regular-expressions is available.</li>
 * <li><strong>regexpi:</strong><em>regexpi</em>: Match a string using a case-insensitive regular-expression.</li>
 * <li><strong>exact:</strong><em>string</em>: Match a string exactly, verbatim, without any of that fancy
 * wildcard stuff.</li>
 * </ul>
 * <p>
 ...
 * specified once, as usual, at the beginning of the pattern.
 * </p>
 */
public abstract class BrowserDriver {
	 ...
	 * <p>
	 * Option locators provide different ways of specifying options of an HTML Select element (e.g. for
	 * selecting a specific option, or for asserting that the selected option satisfies a specification).
	 * There are several forms of Select Option Locator.
	 * </p>
	 * <ul>
	 * <li><strong>label</strong>=<em>labelPattern</em>: matches options based on their labels, i.e. the
	 ...
	 * @param expectedText
	 */
	public abstract void assertLabelContainsText(TextLabel element, String expectedText);
	 ...
	 * <code>this</code> will refer to the Selenium object. Use <code>window</code> to refer to the window of
	 * your application, e.g. <code>window.document.getElementById('foo')</code>
	 * </p>
	 * <p>
	...
	public void waitForElementVisible(String locator) {
		for (int second = 0;; second++) {
			if (second >= Integer.valueOf(BrowserDriver.STANDARD_DHTML_LOAD_WAIT_TIME) /
					StopWatch.MS_PER_SECOND)
				fail("Timeout waiting for element to become visible. locator: " + locator);
	...
	public void waitForElement_NOT_Visible(String locator) {
		for (int second = 0;; second++) {
			if (second >= Integer.valueOf(BrowserDriver.STANDARD_DHTML_LOAD_WAIT_TIME) /
					StopWatch.MS_PER_SECOND)
				fail("Timeout waiting for element to become not visible: " + locator);
	...
	public void delay() {
		if (useDelay) {
			int delayTime = minDelay + (int) (randomizer.nextFloat() * (maxDelay - minDelay));
			sleep(delayTime);
		}
	}
