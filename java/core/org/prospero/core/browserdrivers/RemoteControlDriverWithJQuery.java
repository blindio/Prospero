...
public class RemoteControlDriverWithJQuery extends RemoteControlDriver {
	...
	protected static Logger logger = Logger.getLogger(RemoteControlDriverWithJQuery.class);
	
	// private static final int MS_PER_SECOND = 1000;
	// public static final String CSS_PREFIX = "css=";
	
	protected RemoteControlDriverWithJQuery(String host, int port, String startCommand, String url) {
		super(host, port, startCommand, url);
	}
	
	public boolean isElementVisible(String cssLocator) {
		injectJqueryIfAbsent();
		return Boolean.valueOf(executeJavascript(JqueryCodeFactory.getVisibilityCode(cssLocator)));
	}
	...
	public void check(String locator) {
		getSelenium().check(LocatorStrategyIdentifier.prepareforSelenium(locator));
	}
	
	public boolean isElementPresent(String locator) {
		if (LocatorStrategyIdentifier.isNonSeleniumCss(locator))
			return isElementVisible(locator);
		return getSelenium().isElementPresent(LocatorStrategyIdentifier.prepareforSelenium(locator));
	}
	
	public String getValue(String locator) {
		return getSelenium().getValue(LocatorStrategyIdentifier.prepareforSelenium(locator));
	}
	...
	public String getText(String locator) {
		...
		return getSelenium().getText(LocatorStrategyIdentifier.prepareforSelenium(locator));
	}
	
	public boolean isChecked(String locator) {
		return getSelenium().isChecked(LocatorStrategyIdentifier.prepareforSelenium(locator));
	}
	
	public void select(String locator, String selection) {
		getSelenium().select(LocatorStrategyIdentifier.prepareforSelenium(locator), selection);
		
	}
	
	public void type(String locator, String entry) {
		getSelenium().type(LocatorStrategyIdentifier.prepareforSelenium(locator), entry);
		
	}
	
	public void selectFrame(String locator) {
		getSelenium().selectFrame(LocatorStrategyIdentifier.prepareforSelenium(locator));
	}
	...
	public void injectJqueryIfAbsent() {
		...
		if (jqueryDoesNotExistOnPage()) {
			String jqueryLibraryString = JqueryCodeFactory.getJqueryLibraryString();
			executeJavascript(jqueryLibraryString);
		}
	}
	
	private boolean jqueryDoesNotExistOnPage() {
		logger.debug("returns = " + getSelenium().getEval(JqueryCodeFactory.getJqueryUndefinedString()));
		return Boolean.valueOf(getSelenium().getEval(JqueryCodeFactory.getJqueryUndefinedString()));
	}
	...
	public void typeKeys(String locator, String searchString) {
		getSelenium().typeKeys(LocatorStrategyIdentifier.prepareforSelenium(locator), searchString);
	}
