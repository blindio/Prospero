...
public class PhantomJSInstaller {
	...
	public static String getPhantomJSExecutablePath() {
		return getPhantomJSInstallDirPath() + PhantomJSArchiveFile.getExecutablePath();
	}
	
	public static String getPhantomJSInstallDirPath() {
		return SystemUtils.getUserDir().getAbsolutePath() + SystemUtils.FILE_SEPARATOR 
				+ PHANTOMJS_INSTALL_DIR + SystemUtils.FILE_SEPARATOR;
	}
	
	
	private enum PhantomJSArchiveFile {
		WINDOWS(PhantomJSArchiveFile.WINDOWS_OS, PhantomJSArchiveFile.WINDOWS_EXEC_PATH),
		MAC(PhantomJSArchiveFile.MAC_OS, PhantomJSArchiveFile.UNIX_EXEC_PATH),
		LINUX_32(PhantomJSArchiveFile.LINUX_32_OS, PhantomJSArchiveFile.UNIX_EXEC_PATH),
		LINUX_64(PhantomJSArchiveFile.LINUX_64_OS, PhantomJSArchiveFile.UNIX_EXEC_PATH);
		...
		private static String getArchiveName() {
			return PhantomJSInstaller.PHANTOMJS_FILE_PREFIX + FILENAME_SEPARATOR + PHANTOMJS_VERSION
		...
		private static String getExecutablePath() {
			return getArchiveName() + SystemUtils.FILE_SEPARATOR + getPhantomJSFile().execPath;
		}
		...
		private static PhantomJSArchiveFile getPhantomJSFile() {
			if (phantomJSFile == null) {
				if (SystemUtils.IS_OS_WINDOWS) {
					...
					throw new ProsperoRuntimeAutomationException("Unsupported Operating System");
				}
			}
	...
	private enum ArchiveFormat {
		BZIP2(new TarBZip2UnArchiver(), ArchiveFormat.ARCHIVE_FILE_EXT_TARBZ2),
		ZIP(new ZipUnArchiver(), ArchiveFormat.ARCHIVE_FILE_EXT_ZIP);
